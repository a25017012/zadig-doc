...
permalink: /v1.13.0/install/all-in-one/
---
::: warning
...
```bash
# 下载安装脚本并添加可执行权限
curl -LO https://github.com/koderover/zadig/releases/download/v1.13.0/all_in_one_install_quickstart.sh
...
当看到如图的输出时，说明安装已经完成。
如果中间安装失败 想第二次执行安装 ，请 执行 quickstart
## 步骤 4：验证安装结果
...
curl -SsL https://github.com/koderover/zadig/releases/download/v1.13.0/uninstall.sh |bash
```
